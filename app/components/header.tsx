    ...
    <>
      {!isMenuOpen ? (
        <header className="container-sm mx-auto">
          <div className="">
            <div className="flex items-center justify-between bg-[#00000069] backdrop-blur-xl w-[359px] lg:w-[1240px] h-[56px] lg:h-[96px] rounded-full fixed left-0 right-0 top-[30px] mx-auto px-7 lg:px-14 z-50 ">
              <div>
                <Logo />
              </div>
              ...
              <div className="hidden lg:block">
                <Button type="button" style="primary" css="w-[182px] h-[48px] bg-[#B81A14] rounded-[5px]">Order Now</Button>
              </div>
            </div>
          </div>
        </header>
            ...
            <ul className="flex flex-col items-start gap-y-[40px] text-[20px] font-bold">
              {NavLinks?.map((l, i) => (
                <li key={i} className="text-black">
                  <Link href={l.router} onClick={toggleMenu}>{l.title}</Link>
                </li>
              ))}
              <li className="">
                <Button type="button" style="primary" css="w-[182px] h-[48px] bg-[#B81A14] rounded-[5px]">Order Now</Button>
